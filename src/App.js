...
class App extends Component 
{
    ...
    move(e)
    {
        document.addEventListener('mousemove', this.moveYearOne);
    ...
    move2(e)
    {
        ...
        document.removeEventListener('mousemove', this.moveYearOne);
    }
                    ...
                    <div className='yearSelectionContainer'>
                        <div>
                            <YearBar 
                                ...
                                lower={1981}
                                upper={2018}
                                fetchArticles={(y, year)=>this.fetchArticles(y, year)}
                            />
                        </div>
                        <div>
                            <YearBar 
        ...
        if(!isNaN(e.target.value) || e.target.value === '')
        {
            ...
            else
            {
                this.setState({
                    ...
                    lower: 'yearLimit lower ',
                    upper: 'yearLimit upper '
                }, ()=>this.props.fetchArticles(this.props.type+'Year', this.state.year));
    ...
    render ()
    {
        return(
            <div className='yearSelect'>
                <h4 className='yearSelectHeader'>{this.props.type === 'lesser' ? 'Select first year' : 'Select second year'}</h4>
                <div className='yearInputContainer'>
                    <p className={this.state.lower}>{this.props.lower-1}{' <'}</p>
                    <input
                        className='yearInput'
                        value={this.state.year}
                        onChange={(e)=>this.updateYear(e)}
                    />
                    <p className={this.state.upper}>{'> '}{this.props.upper+1}</p>
                </div>
...
class Article extends Component
{
    ...
    fetchArticle()
    {
        ...
        const base = 'https://us-central1-personalwebsite-189620.cloudfunctions.net/requestAnArticle';
        ...
        request.onload = ()=>{
            ...
            if(request.status === 200)
            {
                ...
                console.log(data);
                let res = data[0];
                let body = this.formatBody(res['blocks']['body']['0']['bodyHtml']);
                this.setState({
                    title: res.webTitle,
                    date: res.webPublicationDate.slice(0,10),
                    author: res['tags']['0'] ? res['tags']['0']['webTitle'] : 'Author Unavaliable',
                    body: body,
                });
            }
            else
    ...
    render()
    {
        return(
            ...
            >
                ...
                <h4 className='cardSection'>Sub-Section: {this.props.subSection}</h4>
            </div>
        );
    }
}
